...
func (s *Standalone) renderChrome(width, _ int, content string) string {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(s.th.Accent).Render("sextant daemon logs")
...
func (s *Standalone) renderStatus(width int) string {
	...
	left := "daemon log"
	if s.inner.Following() {
		left += "  ▼ following"
	}
	right := "[j/k] scroll  [g/G] top/bottom  [q] quit"
