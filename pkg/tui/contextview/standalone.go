...
// `sextant agents context <agent> -i`.
...
func (s *Standalone) renderChrome(width, _ int, content string) string {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(s.th.Accent).Render("sextant agents context")
...
func (s *Standalone) renderStatus(width int) string {
	...
	left := fmt.Sprintf("mode=%s", s.inner.Mode())
	if s.inner.sv.Following() {
		left += "  ▼ following"
	}
	right := s.modeTabs() + "  [q] quit"
...
func (s *Standalone) modeTabs() string {
	parts := make([]string, len(sessionlog.AllModes))
	for i, mode := range sessionlog.AllModes {
		parts[i] = fmt.Sprintf("%d:%s", i+1, mode)
	}
