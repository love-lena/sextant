...
// NewStandalone wraps m and fires an initial LoadMsg for traceID so the
// model loads via the same path the dash uses when routing.
func NewStandalone(m *Model, traceID string) *Standalone {
	...
	if traceID != "" {
		opts = append(opts, component.WithInitialLoad(traceID))
	}
...
func (s *Standalone) renderChrome(width, _ int, content string) string {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(s.th.Accent).Render("sextant traces")
...
func (s *Standalone) renderStatus(width int) string {
	...
	left := fmt.Sprintf("trace=%s", shortTrace(s.inner.TraceID()))
	right := "[j/k] nav  [⏎] expand/collapse  [esc] collapse  [q] quit"
