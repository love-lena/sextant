...
func (s *Standalone) renderChrome(width, _ int, content string) string {
	...
	title := lipgloss.NewStyle().Bold(true).Foreground(s.th.Accent).Render("sextant audit")
...
func (s *Standalone) renderStatus(width int) string {
	...
	left := fmt.Sprintf("rows=%d  (last 24h)", s.inner.Count())
	right := "[j/k] nav  [⏎] detail  [r] refresh  [/] filter  [q] quit"
