...
for f in tweaks-panel artifact home sidebar artifacts review conversations goals mobilize workflow workengine app; do
  "${ESBUILD[@]}" "$DIR/$f.jsx" \
    --jsx=transform \
    --jsx-factory=React.createElement --jsx-fragment=React.Fragment \
    --outfile="$DIR/$f.js" --log-level=warning
done
echo "built dash UI components → $DIR/{tweaks-panel,artifact,home,sidebar,artifacts,review,conversations,goals,mobilize,workflow,workengine,app}.js"
