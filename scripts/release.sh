#!/usr/bin/env bash
# Build the release tarballs for a tag: the three binaries cross-compiled per
...
#   bin/{sextant,sextant-dash,sextant-mcp}
...
for p in "${platforms[@]}"; do
  ...
  out="dist/${name}"
  mkdir -p "${out}/bin"
  for cmd in sextant sextant-dash sextant-mcp; do
