// The real-bus test harness for the review co-equality scenario (TASK-239 AC#3):
// it stands up the actual Go `sextant` bus as a subprocess, mints scoped creds, and
// builds the Go convention helper (test/gohelper) — so the scenario exercises the
// REAL Go review convention against the REAL TS review convention on ONE bus, not a
// mock. Mirrors the goals convention's harness (conventions/goal/ts/test/harness.ts).
//
...
function ensureBinary(store: string): string {
  ...
  const root = repoRoot();
  const ui = spawnSync("bash", [join(root, "scripts", "build-dash-ui.sh")], { cwd: root, encoding: "utf8" });
  if (ui.status !== 0) {
    throw new Error(`build-dash-ui.sh failed (status ${ui.status}):\n${ui.stdout}\n${ui.stderr}`);
  }

  const bin = join(store, "sextant");
  const build = spawnSync("go", ["build", "-o", bin, "./clients/sextant-cli"], { cwd: root, encoding: "utf8" });
  if (build.status !== 0) {
...
// buildGoHelper compiles the Go review convention helper (test/gohelper) into the
// store dir. It drives the REAL Go review convention so the co-equality proof is
// genuine.
function buildGoHelper(store: string): string {
  const root = repoRoot();
  const bin = join(store, "gohelper");
  const build = spawnSync("go", ["build", "-o", bin, "./conventions/review/ts/test/gohelper"], {
    cwd: root,
    encoding: "utf8",
  });
  if (build.status !== 0) {
...
export function startBus(): Bus {
  const store = mkdtempSync(join(tmpdir(), "sextant-ts-review-e2e-"));
