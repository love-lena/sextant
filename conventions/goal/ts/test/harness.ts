...
// Mirrors the SDK package's harness (sdk/ts/test/harness.ts) and the
...
function ensureBinary(store: string): string {
  ...
  // compile so the go:embed in clients/sextant-dash/dashapi resolves.
...
function buildGoHelper(store: string): string {
  ...
  const build = spawnSync(
    "go",
    ["build", "-o", bin, "./conventions/goal/ts/test/gohelper"],
