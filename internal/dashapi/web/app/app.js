(function() {
  ...
  function App() {
    ...
    const convList = useMemo(() => Object.entries(convos).filter(([subj]) => !subj.startsWith("msg.topic.artifact.")).sort((a, b) => (b[1].last || 0) - (a[1].last || 0)).map(([subj, c]) => {
      ...
      if (subj.startsWith("msg.client.")) {
        ...
        other = ids.find((x) => x !== self.id) || ids[0] || "";
        type = "dm";
        name = nameOf(other);
      }
