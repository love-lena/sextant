...
| [0036](0036-presence-and-liveness-derive-from-a-client-heartbeat.md) | Presence and liveness derive from a client heartbeat | proposed |
| [0037](0037-subscriptions-and-context-survive-a-session-resume.md) | Subscriptions and the active context survive a session resume | proposed |
...
- **0037 — subscriptions and the active context survive a session resume** — *proposed* (TASK-124; the MCP adapter persists a session's manual subscriptions + each one's last-delivered seq + the `context_use` choice, keyed on the session id beside the attest cursor, and restores them on every connect — re-pin the context before auto-mint, re-subscribe + catch up by seq — so a resume/compaction/restart self-heals instead of silently dropping delivery. An adapter convention over `message_read` + `message_subscribe`; epoch unchanged; retires the interim keepalive. A seq-gap liveness watchdog composes with the 0036 heartbeat as a following slice)
