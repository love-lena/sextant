...
| [0035](0035-goals-are-shared-objectives-tracked-by-observation.md) | Goals are shared objectives, tracked by observation | proposed |
...
- **0035 — goals are shared objectives, tracked by observation** — *proposed* (TASK-84; the `goal` latest-value artifact `goal.<id>` + the `goal.update` signal on `msg.topic.goals`, mirroring `agent.status`; the tracker signals movement and the owner/operator stay authoritative — signal + cooperate, never track + manage; Go + dash deferred to the TASK-87 tracker)
