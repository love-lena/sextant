...
### func `(*Client) MintSession`

```go
func (c *Client) MintSession(ctx context.Context) (IssuedClient, error)
```

MintSession asks the bus to mint a short-lived SESSION credential for THIS client's OWN identity (ADR-0044): a fresh, TTL-bounded credential that authenticates as the same id — the same unforgeable author, the same DM/inbox space — but with the privileged issuance ops denied. The dash uses it to hand a browser tab a credential that acts AS the operator, so the page's DMs, DM history, and self-authorship are the operator's, without the operator's perpetual key ever reaching the browser. The returned creds are secret material (they ride this client's own inbox); the id is this client's own id.
...
PublishResult is the bus-stamped outcome of a PublishMsg call: the bus-minted frame id and the stream sequence at which the frame landed. It is the SDK-owned value the public boundary returns — callers name this, never the internal wire type (TASK-182).
