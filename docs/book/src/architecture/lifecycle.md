...
- `defined → running`: `sextant agents create` (or `spawn_agent` MCP tool).
- `running → defined`: `sextant agents stop` — stops the container, leaves the definition.
...
1. Operator runs `sextant agents create <name> --template <T>` (`cmd/sextant/agents.go`).
