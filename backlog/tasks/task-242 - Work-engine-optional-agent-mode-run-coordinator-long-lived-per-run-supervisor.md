...
- [ ] #4 Wrapper, not author: substantive work is delegated to dispatched workers; a fundamentally-wrong output triggers redo-with-feedback (re-dispatch), never the agent authoring the deliverable. edit-then-advance is bounded by a CONCRETE, checkable limit (MUST be defined before build — e.g. a declared field/diff-size cap); anything larger MUST redo. Identity is ULID+function. Proof: a major-change scenario shows a dispatched worker did the redo, not the agent; the agent has no deliverable-authoring tools. Fake-pass guard: the agent becoming the worker (writing the deliverable itself) fails.
- [ ] #5 Observable: the coordinator-agent's turns/thinking/decisions stream to its agent.activity feed and render live in the dash — >=1 frame incl. turn_end per reviewed step. Proof: the dash shows the coordinator-agent's per-step activity during an agent-mode run. Fake-pass guard: a silent agent (no activity per reviewed step) fails.
- [ ] #6 Agent decisions do NOT bypass the deterministic floor (TASK-243): an agent advance / edit-then-advance still passes the proof-gate — a run cannot reach done over a declared deliverable that does not exist, even on 'advance'. Proof: an agent-mode run whose brief declares a phantom proof artifact ends BLOCKED despite an advance decision. Fake-pass guard: agent mode routing around the proof-gate fails.
<!-- AC:END -->
