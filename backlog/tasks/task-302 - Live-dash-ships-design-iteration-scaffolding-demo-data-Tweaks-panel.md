...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The shipped dash renders ONLY live bus data — an empty bus shows empty views (no seed goals/artifacts/agents); SNAPSHOT and the snapshot/blank data-mode are gone from the operator build
- [x] #2 No Tweaks panel in the operator dash; the chosen design values are baked as fixed constants (accent #4f9d68, sidebar left/paper/sections, live-pulse on) so there is no visual change
- [x] #3 Verified live: with zero goals on the bus, the Goals page reads '0 goals' (or equivalent empty state), not the demo fixtures
<!-- AC:END -->
...
Done in PR #310: removed the SNAPSHOT demo dataset, the Tweaks panel, the snapshot/blank data-mode, and the EDITMODE host hooks from the dash; views read live bus data directly; design values baked as constants (no visual change). Verified live via /rc dash on the operator's bus — Goals renders the real empty state (no fixtures), no Tweaks panel. Reaches the operator's installed dash on the next release (the 'release' label tracks that).
