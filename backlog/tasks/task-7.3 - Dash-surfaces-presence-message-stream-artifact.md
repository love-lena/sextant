...
The surface stratum (ADR-0021): a Surface contract (set size, focus, render content, emit OpenMsg/DoneMsg, declare id+title for toggling) and the three M4 panes — presence (client records), message-stream (one read surface + optional compose; round-trip merge), artifact (document reader + review). Built on the toolkit + adapter, touching only the public SDK.
