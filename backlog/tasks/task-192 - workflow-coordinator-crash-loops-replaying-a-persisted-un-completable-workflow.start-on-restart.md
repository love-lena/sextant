...
<!-- SECTION:NOTES:BEGIN -->
Fixed: newStartConsumer subscribed with DeliverAll, replaying every historical workflow.start on each (re)start — stale ones (no dispatcher → no spawn.ack → 90s timeout → drain → respawn) crash-looped the listen-mode coordinator. Now subscribes new-only (a workflow.start is a live command); the per-workflow coordinator's DeliverAll resume is unchanged. Regression test TestStartConsumer_IgnoresHistoricalStart (race-clean). v0.7.x.
<!-- SECTION:NOTES:END -->
