...
// legitimately reaches the SDK (surface → busfeed → sdk/go). The line
...
func TestImportClosure(t *testing.T) {
	const pkg = importcheck.Module + "/clients/sextant-tui/internal/tui/layout"
	importcheck.AssertSDKOnly(t, pkg)
	importcheck.AssertNoDirectImport(
		t, pkg,
		importcheck.Module+"/sdk/go",
		importcheck.Module+"/clients/sextant-tui/internal/tui/busfeed",
	)
}
