...
// presentation code — no SDK, no protocol binding, no bus, no NATS, and no
// in-module package but a sibling TUI stratum — and the closure check holds
// that even through an allowed project import. It
// checks the non-test package, so a test harness import can never widen the
// production allowlist.
func TestImportClosure(t *testing.T) {
	importcheck.AssertPresentationOnly(t, importcheck.Module+"/clients/sextant-tui/internal/tui/theme")
}
