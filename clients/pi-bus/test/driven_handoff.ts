...
// the REAL recipe (clients/dispatcher/recipes/pi.sh) to launch a headless
...
// NOT a fixed number of "..": the built file lives at clients/pi-bus/dist/test/, so
// a hard-coded relative count overshoots/undershoots and the recipe path comes out
// wrong. The go.mod walk is
// correct from src/ AND dist/. SEXTANT_REPO_ROOT still overrides if set.
const REPO_ROOT = repoRoot();
const RECIPE = join(REPO_ROOT, "clients", "dispatcher", "recipes", "pi.sh");
