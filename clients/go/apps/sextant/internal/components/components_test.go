...
func TestRecipeDriftGuard(t *testing.T) {
	...
	// apps/sextant/internal/components -> repo root is six segments up, then to
	// the dispatch app's reference recipe.
	source, err := os.ReadFile(filepath.Join("..", "..", "..", "..", "..", "..", "clients", "go", "apps", "dispatch", "recipes", "agent.sh"))
	if err != nil {
		t.Fatalf("read source recipe: %v", err)
	}
	if string(embedded) != string(source) {
		t.Fatalf("embedded recipe has drifted from clients/go/apps/dispatch/recipes/agent.sh — re-copy it into the components embed dir")
	}
}
