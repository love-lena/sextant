...
(function () {
  ...
  // ---- minimal #bus hash route (TASK-220 will replace this with the shell nav) ----
  // When the URL hash is #bus, mount the inspector full-screen over its own root so
  // the surface is reachable + testable WITHOUT touching app.jsx / sidebar.jsx (the
  // app shell is being rebuilt in parallel). Toggling the hash away unmounts it.
  function mountRoute() {
    const wantBus = location.hash === "#bus";
    let host = document.getElementById("sx-bus-route");
    if (wantBus) {
      if (!host) {
        host = document.createElement("div");
        host.id = "sx-bus-route";
        host.className = "sx-bus-route";
        document.body.appendChild(host);
      }
      const root = ReactDOM.createRoot ? (host._root || (host._root = ReactDOM.createRoot(host))) : null;
      if (root) root.render(React.createElement(BusInspector, {}));
      else ReactDOM.render(React.createElement(BusInspector, {}), host); // React 17 fallback
    } else if (host) {
      if (host._root && host._root.unmount) host._root.unmount();
      else if (ReactDOM.unmountComponentAtNode) ReactDOM.unmountComponentAtNode(host);
      host.remove();
    }
  }
  window.addEventListener("hashchange", mountRoute);
  // Defer the initial mount until the DOM + the bus data layer are present.
  if (document.readyState !== "loading") mountRoute();
  else document.addEventListener("DOMContentLoaded", mountRoute);
})();
